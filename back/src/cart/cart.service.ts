...
export class CartService {
  constructor(
    ...
    @InjectRepository(Products) private readonly productsRepo: Repository<Products>, // ajusta a Product si aplica
  ) {}
  ...
  private async recalc(cartId: string, touch = false) {
    ...
    const subtotal = cart.items.reduce((acc, it) => acc + Number(it.unitPriceCurrent) * it.quantity, 0);
  ...
  async getOrCreateCart(ctx: { userId?: string; cookieCartId?: string }) {
    ...
    if (ctx.userId) {
      ...
      if (cart) return cart;
      cart = this.cartsRepo.create({ user: { id: ctx.userId } as any, status: 'ACTIVE', items: [] });
  ...
  async addItem(cart: Cart, dto: AddItemDto) {
    ...
    if (product.stock < dto.quantity) {
      throw new ConflictException({ message: 'Insufficient stock', stock: product.stock });
    }
  ...
  async updateQuantity(cart: Cart, itemId: string, dto: UpdateQuantityDto) {
    const item = await this.itemsRepo.findOne({ where: { id: itemId }, relations: ['product', 'cart'] });
  ...
  async removeItem(cart: Cart, itemId: string) {
    const item = await this.itemsRepo.findOne({ where: { id: itemId }, relations: ['cart'] });
    if (!item || item.cart.id !== cart.id) throw new NotFoundException();
    await this.itemsRepo.remove(item);
    await this.recalc(cart.id);
    return this.load(cart.id);
  ...
  async refresh(cartId: string) {
    ...
    for (const it of cart.items) {
      ...
      if (it.unitPriceCurrent !== priceNow) {
        changes.push({ itemId: it.id, type: 'PRICE_CHANGED', old: it.unitPriceCurrent, new: priceNow });
        it.unitPriceCurrent = priceNow;
      }

      // stock
      if (p.stock < it.quantity) {
        changes.push({ itemId: it.id, type: 'STOCK_REDUCED', requested: it.quantity, available: p.stock });
        it.isValid = false;
      } else {
        it.isValid = true;
      }
